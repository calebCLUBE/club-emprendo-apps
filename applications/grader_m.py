...
def grade_from_dataframe(
    ...
    out_df, removed = _dedupe_scored_rows(
        out_df,
        "score",
        ["email", "correo_electronico", "cedula", "id_number", "ID"],
    )
    if removed and log_fn:
        log_fn(
            f"→ Removed {removed} duplicate mentora row(s) after grading, keeping the highest score per person"
        )
    return out_df
