...
def yesish(v) -> bool:
    """
    Returns True for:
      - 'sí', 'si' (any casing / with accents / inside longer text)
      - 'yes' (any casing / inside longer text)
      - 'true', '1', 'y' (common boolean-ish)
    """
    if v is None:
        return False
...
def autograde_and_email_emprendedora_a1(request, app: Application):
    ...
    Reads answers from DB (works for master + group clones).
    Keys may vary depending on which form build ran, so we check both the new
    e1_* slugs and older generic slugs.
    """
    answers = {
