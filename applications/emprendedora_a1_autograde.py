...
def yesish(v) -> bool:
    """
    EXACT match to Apps Script:
      normalize("NFD").replace accents, lower, trim, then `includes("si")`.
    """
    t = str(v or "")
...
def build_approved_html(form_2_link: str) -> str:
    # EXACT HTML template from Apps Script, only swap FORM_2_LINK with form_2_link.
    return (
...
def build_rejected_html() -> str:
    # EXACT HTML template from Apps Script.
    return (
        '<div style="font-family:Arial,Helvetica,sans-serif;line-height:1.6;max-width:700px;margin:0 auto;word-break:break-word;white-space:normal;">'
...
def autograde_and_email_emprendedora_a1(*, request, application, answers_by_slug: dict) -> str:
    """
    Mirrors Apps Script grading:
      passesRequisitos = yesish(requisitos)
      passesDisponibilidad = yesish(disponibilidad)
      hasEmprendimiento = yesish(emprendimiento)
    ...
    emprendimiento = answers_by_slug.get("business_active", "")
    ...
    if passes_requisitos and passes_disponibilidad and has_emprendimiento:
        # ✅ Eligible — same token flow as Mentora A1
        ...
        form2_link = request.build_absolute_uri(
            reverse("apply_emprendedora_second", args=[application.invite_token])
        )
