...
class ApplicationDraftTrackingTests(TestCase):
    ...
    def test_group_dashboard_separates_rejected_and_shows_progress_funnel(self):
        ...
        self.assertContains(response, 'id="edit-reminder-email-button"')
        self.assertContains(response, "Edit reminder email</button>")
        self.assertContains(response, 'id="reminder-email-editor"')
...
class MultipleChoiceGridTests(TestCase):
    ...
    def test_grid_keeps_semantic_row_and_column_table(self):
        form = build_application_form(self.form_def.slug)()
        html = str(form["q_area_grid"])

        self.assertIn('class="ce-grid-question__table"', html)
        self.assertEqual(html.count('scope="col"'), 3)
        self.assertEqual(html.count('scope="row"'), 2)
        self.assertNotIn("ce-grid-choice__mobile-label", html)
...
class DropboxSignWebhookActaAutomationTests(TestCase):
    ...
    def test_passive_check_ignores_other_group_range_titles(self):
        ...
        participant_list = GroupParticipantList.objects.create(
            group=target_group,
            mentoras_sheet_rows=[self._mentora_row(1, "target@example.com")],
        )
        requests = [
            SignatureRequestLite(
                signature_request_id="wrong-group-request",
                bulk_send_job_id="wrong-job",
                title="G16-17 Acta de compromiso",
                created_at=None,
                signer_emails=["target@example.com"],
                signed_emails=["target@example.com"],
            ),
            SignatureRequestLite(
                signature_request_id="target-group-request",
                bulk_send_job_id="target-job",
                title="G16-18 Acta de compromiso",
                created_at=None,
                signer_emails=["target@example.com"],
                signed_emails=["target@example.com"],
        ...
        output = StringIO()
        with patch.object(Command, "_fetch_signature_requests", return_value=requests), patch.object(
            Command,
        ...
        status = ParticipantEmailStatus.objects.get(email="target@example.com")
